...
GEMINI_KEY = os.getenv("GEMINI_API_KEY", "AIzaSyA7R_sPBOAjv2WLx_A53kNtvY")   
...
class FAISSRAG(object):
    ...
    def __init__(self, generation_llm: OllamaLLM):
        ...
        try:
            print("Initiating FAISS RAG setup...")
    ...
    def timed_rag_generation(self, query: str, top_k: int) -> (str, float, float):
        ...
        if not self.retriever:
            return "Internal Error: FAISS retriever not set up.", 0.0, 0.0
...
class Pipeline(Blueprint):
    ...
    def __init__(self):
        ...
        try:
            self.faiss_rag = FAISSRAG(self.ollama_llm)
    ...
    def benchmark_rag(self, test_cases: List[TestCase], top_k_values: List[int], n_runs: int = 1):
        ...
        """
        print("\n" + "="*60)
        print(f"STARTING RAG BENCHMARK (FAISS + Ollama/{OLLAMA_MODEL})")
        print(f"Similarity Threshold: {ACCURACY_THRESHOLD}")
        print("="*60)
        
        
        print(f"Knowledge Base Size (Raw): {self.faiss_rag.total_docs_size_bytes / (1024**2):.2f} MB")
        print(f"Indexing Time: {self.faiss_rag.indexing_time:.4f}s")
        print("-" * 60)
        ...
        if not self.faiss_rag or not self.ollama_llm:
            print("Cannot run full benchmark: RAG components or Ollama LLM failed to initialize.")
            return
            
        embeddings_model = self.faiss_rag.embeddings
        ...
        for k in top_k_values:
            ...
            for test_case in test_cases:
                for _ in range(n_runs):
                    ...
                    final_answer, retrieval_latency, generation_latency = self.faiss_rag.timed_rag_generation(
    ...
    def pipelines(self) -> List[dict]:
        ...
        return [
            {
                "id": "faiss-ollama-rag-model",
                "name": f"FAISS RAG ({OLLAMA_MODEL} Local)",
                "description": "FAISS RAG pipeline using a local Ollama model for generation."
            }
        ]
    ...
    def pipe(self, user_message, model_id, messages, body):
        ...
        if not self.faiss_rag:
            yield "RAG is unavailable. Check pipeline server logs for Ollama or FAISS errors during startup."
            return
            
        response_content = self.faiss_rag.generate_completion(messages=messages, model_id=model_id, stream=False)
