...
try:
    ...
    from langchain.text_splitter import RecursiveCharacterTextSplitter # Correct name
    ...
    print("\nInstall required packages: pip install qdrant-client langchain-community langchain-ollama langchain numpy scikit-learn pydantic sentence-transformers")
    sys.exit(1) 


GEMINI_KEY = os.getenv("GEMINI_API_KEY", "AIzaSy...") 
...
QDRANT_COLLECTION_NAME = "ollama_rag_collection" 
...
def _format_context_prompt(query: str, retrieved_docs: List[Document]) -> str:
...
class QdrantRAG(object):
    def __init__(self, generation_llm: OllamaLLM):
        self.name="qdrant-rag-model"
        ...
        try:
            print("Initiating QDRANT RAG setup...")
            ...
            print(f"Error during QdrantRAG initialization: {e}")
            raise e

    def timed_rag_generation(self, query: str, top_k: int) -> (str, float, float):
        """
        Performs timed RAG generation (retrieval + generation) using the internal Ollama LLM.
        """
        if not self.retriever:
            return "Internal Error: Qdrant retriever not set up.", 0.0, 0.0
...
class Pipeline(Blueprint):
    ...
    def __init__(self):
        ...
        try:
            self.qdrant_rag = QdrantRAG(self.ollama_llm)
            print("Pipeline instance created! Qdrant RAG Setup Complete.")
            
           
            self.benchmark_rag(
                test_cases=[
                    TestCase(
                        query="What are the essential components of a good greeting?",
                        gold_answer="A good greeting typically includes eye contact, a slight smile, and appropriate verbal affirmation."
                    ),
                    TestCase(
                        query="Summarize the importance of RAG in modern LLMs.",
                        gold_answer="RAG helps LLMs by providing up-to-date, domain-specific information, reducing hallucinations, and offering source attribution."
                    ),
                ],
            ...
            print("RAG Pipeline initialization failed during Qdrant setup or document loading.")
            traceback.print_exc()
            self.qdrant_rag = None
            raise e
    ...
    def benchmark_rag(self, test_cases: List[TestCase], top_k_values: List[int], n_runs: int = 1):
        ...
        """
        print("\n" + "="*60)
        print(f"STARTING RAG BENCHMARK (Qdrant + Ollama/{OLLAMA_MODEL})")
        print(f"Similarity Threshold: {ACCURACY_THRESHOLD}")
        print("="*60)
        
        if not self.qdrant_rag:
            print("Cannot run full benchmark: Qdrant RAG component failed to initialize.")
            return
            
        print(f"Knowledge Base Size (Raw): {self.qdrant_rag.total_docs_size_bytes / (1024**2):.2f} MB")
        print(f"Indexing Time: {self.qdrant_rag.indexing_time:.4f}s")
        print("-" * 60)
        ...
        embeddings_model = self.qdrant_rag.embeddings
        ...
        for k in top_k_values:
            ...
            for test_case in test_cases:
                for _ in range(n_runs):
                    final_answer, retrieval_latency, generation_latency = self.qdrant_rag.timed_rag_generation(
            ...
            if total_runs > 0:
                ...
                retrieval_qps = 1 / avg_retrieval_time if avg_retrieval_time > 0 else 0
    ...
    def pipelines(self) -> List[dict]:
        ...
        return [
            {
                "id": "qdrant-ollama-rag-model", 
                "name": f"Qdrant RAG ({OLLAMA_MODEL} Local)", 
                "description": "Qdrant RAG pipeline using a local Ollama model for generation."
            }
        ]
    ...
    def pipe(self, user_message, model_id, messages, body):
        ...
        if not self.qdrant_rag:
            yield "RAG is unavailable. Check pipeline server logs for Ollama or Qdrant errors during startup."
            return
            
        response_content = self.qdrant_rag.generate_completion(messages=messages, model_id=model_id, stream=False)
